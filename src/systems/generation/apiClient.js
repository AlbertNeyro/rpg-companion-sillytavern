...
export async function generateWithExternalAPI(messages) {
    ...
    try {
        ...
        if (useSillyTavernProxy) {
            // SillyTavern performs the provider request server-side, avoiding browser CORS.
        ...
        if (!useSillyTavernProxy && error.name === 'TypeError' &&
            ...
            throw new Error(`CORS Access Blocked: ${normalizedBaseUrl} does not allow direct browser access. Enable the SillyTavern backend proxy in RPG Companion and configure the same Custom API URL and API key in SillyTavern.`);
        }
        throw error;
    }
}
